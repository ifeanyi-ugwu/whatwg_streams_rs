    ...
    Sink: WritableSink<T>,
{
    ...
    pub fn new(sink: Sink, strategy: Box<dyn QueuingStrategy<T>>) -> Self {
...
{
    /// Internal constructor for non-Send futures (local execution)
    fn new_with_local_spawn_internal<F>(
        sink: Sink,
        strategy: Box<dyn QueuingStrategy<T>>,
        spawn_fn: F,
    ) -> Self
        ...
        F: FnOnce(futures::future::LocalBoxFuture<'static, ()>),
    {
        ...
        let (ctrl_tx, ctrl_rx) = futures::channel::mpsc::unbounded();
        let controller = WritableStreamDefaultController::new(ctrl_tx.clone());
        ...
        let fut = async move {
            stream_task_local(
                command_rx,
                inner,
                Arc::clone(&backpressure_clone),
                Arc::clone(&closed_clone),
                Arc::clone(&errored_clone),
                Arc::clone(&queue_total_size_clone),
                controller_clone,
                ctrl_rx,
            )
            .await;
    ...
    /// Internal constructor for Send futures (threaded execution)
    fn new_with_send_spawn_internal<F>(
        sink: Sink,
    ...
    /// Local / blocking executor (no Send required)
    pub fn new_local(sink: Sink, strategy: Box<dyn QueuingStrategy<T>>) -> Self {
        Self::new_with_local_spawn_internal(sink, strategy, |fut| {
            futures::executor::block_on(fut);
        })
    }

    /// Custom local executor (user provides spawn for LocalBoxFuture)
    pub fn new_with_local_spawn<F>(
        sink: Sink,
        strategy: Box<dyn QueuingStrategy<T>>,
        spawn_fn: F,
    ) -> Self
    ...
    {
        Self::new_with_local_spawn_internal(sink, strategy, spawn_fn)
    }

    /// Threaded executor (requires Send)
    pub fn new_threaded(sink: Sink, strategy: Box<dyn QueuingStrategy<T> + Send>) -> Self
    where
        Sink: Send + 'static,
        T: Send + 'static,
    {
        Self::new_with_send_spawn_internal(sink, strategy, |fut| {
            std::thread::spawn(move || {
                futures::executor::block_on(fut);
            });
        })
    ...
    pub fn new(sink: Sink, strategy: Box<dyn QueuingStrategy<T> + Send>) -> Self
    where
        Sink: Send + 'static,
        T: Send + 'static,
    {
    ...
    /// Custom threaded executor (user provides spawn for Send BoxFuture)
    pub fn new_with_send_spawn<F>(
        sink: Sink,
        strategy: Box<dyn QueuingStrategy<T> + Send>,
        spawn_fn: F,
    ) -> Self
    where
        ...
        T: Send + 'static,
    {
        Self::new_with_send_spawn_internal(sink, strategy, spawn_fn)
    }
...
pub trait WritableSink<T: 'static>: Sized {
    ...
    ) -> impl Future<Output = StreamResult<()>> + 'a + Send;
...
pub trait __WritableSink<T: 'static>: Sized {
    type StartFut<'a>: Future<Output = StreamResult<()>> + 'a
    where
        Self: 'a;

    type WriteFut<'a>: Future<Output = StreamResult<()>> + 'a
    where
        Self: 'a;

    type CloseFut: Future<Output = StreamResult<()>> + 'static;

    type AbortFut<'a>: Future<Output = StreamResult<()>> + 'a
    where
        Self: 'a;
    ...
    fn close(self) -> Self::CloseFut;

    fn abort<'a>(&'a mut self, reason: Option<String>) -> Self::AbortFut<'a>;
}

#[async_trait::async_trait(?Send)]
pub trait _WritableSink<T: 'static>: Sized {
    async fn start(&mut self, ctrl: &mut WritableStreamDefaultController) -> StreamResult<()> {
        Ok(())
    }
...
fn process_command<T, Sink, Strategy: ?Sized + QueuingStrategy<T>>(
    cmd: StreamCommand<T>,
    inner: &mut WritableStreamInner<T, Sink, Strategy>,
    backpressure: &Arc<AtomicBool>,
    closed: &Arc<AtomicBool>,
    errored: &Arc<AtomicBool>,
    queue_total_size: &Arc<AtomicUsize>,
    ...
    Sink: WritableSink<T> + 'static,
{
    match cmd {
        StreamCommand::Write { chunk, completion } => {
            ...
            inner.update_atomic_counters(queue_total_size);
...
// Local boxed future (no Send bound)
//type LocalBoxFuture<'a, T> = Pin<Box<dyn Future<Output = T> + 'a>>;

// For convenience, 'static variant
type LocalBoxFutureStatic<T> = LocalBoxFuture<'static, T>;

//use futures::future::LocalBoxFuture;
/*let fut: LocalBoxFuture<'static, ()> = async move {
    sink.close().await
}.boxed_local();*/
...
enum __InFlight<Sink> {
    Write {
        fut: LocalBoxFutureStatic<(Sink, Result<(), StreamError>)>,
        completion: Option<oneshot::Sender<Result<(), StreamError>>>,
        chunk_size: usize,
    },
    Close {
        fut: LocalBoxFutureStatic<Result<(), StreamError>>,
        completions: Vec<oneshot::Sender<Result<(), StreamError>>>,
    },
    Abort {
        fut: LocalBoxFutureStatic<Result<(), StreamError>>,
        completions: Vec<oneshot::Sender<Result<(), StreamError>>>,
...
async fn stream_task<T, Sink, Strategy: ?Sized + QueuingStrategy<T>>(
    mut command_rx: UnboundedReceiver<StreamCommand<T>>,
    mut inner: WritableStreamInner<T, Sink, Strategy>,
    backpressure: Arc<AtomicBool>,
    closed: Arc<AtomicBool>,
    errored: Arc<AtomicBool>,
    queue_total_size: Arc<AtomicUsize>,
    ...
    Sink: WritableSink<T> + 'static,
{
    ...
    poll_fn(|cx| {
        inner.process_controller_msgs(&mut ctrl_rx);
        inner.update_atomic_counters(&queue_total_size);
            ...
            match command_rx.poll_next_unpin(cx) {
                Poll::Ready(Some(cmd)) => {
                    ...
                    while let Some(completion) = inner.pending_flush_commands.pop() {
                        inner.process_flush_command(completion, &inflight);
                    }
                }
            ...
            if inner.close_requested {
                ...
                if can_close {
                    if let Some(sink) = inner.sink.take() {
                        ...
                        as LocalBoxFutureStatic<Result<(), StreamError>>;*/
...
// Local version (non-Send futures)
enum InFlightLocal<Sink> {
    Write {
        fut: LocalBoxFuture<'static, (Sink, Result<(), StreamError>)>,
        completion: Option<oneshot::Sender<Result<(), StreamError>>>,
        chunk_size: usize,
    },
    Close {
        fut: LocalBoxFuture<'static, Result<(), StreamError>>,
        completions: Vec<oneshot::Sender<Result<(), StreamError>>>,
    },
    Abort {
        fut: LocalBoxFuture<'static, Result<(), StreamError>>,
...
// Send version (Send futures)
enum InFlightSend<Sink> {
    Write {
        fut: BoxFuture<'static, (Sink, Result<(), StreamError>)>,
        completion: Option<oneshot::Sender<Result<(), StreamError>>>,
        chunk_size: usize,
    },
    Close {
...
// Local task function (for single-threaded execution)
async fn stream_task_local<T, Sink, Strategy: ?Sized + QueuingStrategy<T>>(
    mut command_rx: UnboundedReceiver<StreamCommand<T>>,
    mut inner: WritableStreamInner<T, Sink, Strategy>,
    backpressure: Arc<AtomicBool>,
    closed: Arc<AtomicBool>,
    ...
    Sink: WritableSink<T> + 'static,
{
    let mut inflight: Option<InFlightLocal<Sink>> = None;
    ...
    poll_fn(|cx| {
        ...
        loop {
            match command_rx.poll_next_unpin(cx) {
                Poll::Ready(Some(cmd)) => {
                    ...
                    while let Some(completion) = inner.pending_flush_commands.pop() {
                        //inner.process_flush_command(completion, &inflight.as_ref().map(|_| ()));
                        if inner.state == StreamState::Errored {
                            let _ = completion.send(Err(inner.get_stored_error()));
                            break;
                        }
        ...
        if inner.abort_requested {
            ...
            if let Some(inflight_op) = &mut inflight {
                match inflight_op {
                    InFlightLocal::Write { completion, .. } => {
                        if let Some(sender) = completion.take() {
                            let _ = sender.send(Err(StreamError::Aborted(None)));
                        }
                    }
                    InFlightLocal::Close { completions, .. } => {
                        for sender in completions.drain(..) {
                            let _ = sender.send(Err(StreamError::Aborted(None)));
                        }
                    }
            ...
            if inner.close_requested {
                ...
                if can_close {
                    if let Some(sink) = inner.sink.take() {
                        let fut = async move { sink.close().await }.boxed_local();
                        let completions = std::mem::take(&mut inner.close_completions);
                        inflight = Some(InFlightLocal::Close { fut, completions });
                    } else {
                        inner.state = StreamState::Closed;
                        inner.close_requested = false;
                        update_flags(&inner, &backpressure, &closed, &errored);
                ...
                if let Some(mut sink) = inner.sink.take() {
                    ...
                    let fut = async move { sink.abort(reason).await }.boxed_local();
                    let completions = std::mem::take(&mut inner.abort_completions);
                    inflight = Some(InFlightLocal::Abort { fut, completions });
                } else {
                    inner.abort_requested = false;
                    update_flags(&inner, &backpressure, &closed, &errored);
                    for c in inner.abort_completions.drain(..) {
        ...
        if inflight.is_none() && inner.state == StreamState::Writable {
            if let Some(pw) = inner.queue.pop_front() {
                ...
                if let Some(mut sink) = inner.sink.take() {
                    ...
                    inflight = Some(InFlightLocal::Write {
                        fut: async move {
                            let result = sink.write(chunk, &mut ctrl).await;
                            (sink, result)
                        }
            ...
            match inflight_op {
                InFlightLocal::Write {
                    ...
                    Poll::Ready((mut sink, result)) => {
                        ...
                        if inner.abort_requested {
                            let reason = inner.abort_reason.take();
                            let abort_fut = async move { sink.abort(reason).await }.boxed_local();
                            let completions = std::mem::take(&mut inner.abort_completions);
                            ...
                            inflight = Some(InFlightLocal::Abort {
                                fut: abort_fut,
                                completions,
                            });
                        } else {
                ...
                InFlightLocal::Close { fut, completions } => match fut.as_mut().poll(cx) {
                    Poll::Ready(res) => {
                        match &res {
                ...
                InFlightLocal::Abort { fut, completions } => match fut.as_mut().poll(cx) {
                    Poll::Ready(sink_abort_result) => {
                        let notify_result = match sink_abort_result {
                            Ok(()) => Ok(()),
                            Err(sink_error) => {
...
// Send task function (for multi-threaded execution) - exact copy but with Send bounds
async fn stream_task_send<T, Sink, Strategy: ?Sized + QueuingStrategy<T> + Send>(
    mut command_rx: UnboundedReceiver<StreamCommand<T>>,
    mut inner: WritableStreamInner<T, Sink, Strategy>,
    backpressure: Arc<AtomicBool>,
    closed: Arc<AtomicBool>,
    ...
    /*for<'a> Sink::StartFut<'a>: Future<Output = StreamResult<()>> + Send + 'a,
    for<'a> Sink::WriteFut<'a>: Future<Output = StreamResult<()>> + Send + 'a,
    Sink::CloseFut: Future<Output = StreamResult<()>> + Send + 'static,
    for<'a> Sink::AbortFut<'a>: Future<Output = StreamResult<()>> + Send + 'a,*/
{
    let mut inflight: Option<InFlightSend<Sink>> = None;
    ...
    poll_fn(|cx| {
        ...
        // Command processing loop (identical to local)
        loop {
            match command_rx.poll_next_unpin(cx) {
                Poll::Ready(Some(cmd)) => {
                    process_command(
        ...
        if inner.abort_requested {
            ...
            if let Some(inflight_op) = &mut inflight {
                match inflight_op {
                    InFlightSend::Write { completion, .. } => {
                        if let Some(sender) = completion.take() {
                            let _ = sender.send(Err(StreamError::Aborted(None)));
                        }
                    }
                    InFlightSend::Close { completions, .. } => {
                        for sender in completions.drain(..) {
                            let _ = sender.send(Err(StreamError::Aborted(None)));
                        }
                    }
        ...
        if inflight.is_none() {
            if inner.close_requested {
                ...
                if can_close {
                    if let Some(sink) = inner.sink.take() {
                        let fut = async move { sink.close().await }.boxed(); // .boxed() instead of .boxed_local()
                        let completions = std::mem::take(&mut inner.close_completions);
                        inflight = Some(InFlightSend::Close { fut, completions });
                    } else {
                        inner.state = StreamState::Closed;
                        inner.close_requested = false;
                        update_flags(&inner, &backpressure, &closed, &errored);
                ...
                if let Some(mut sink) = inner.sink.take() {
                    ...
                    let completions = std::mem::take(&mut inner.abort_completions);
                    inflight = Some(InFlightSend::Abort { fut, completions });
                } else {
                    inner.abort_requested = false;
                    update_flags(&inner, &backpressure, &closed, &errored);
                    for c in inner.abort_completions.drain(..) {
        ...
        if inflight.is_none() && inner.state == StreamState::Writable {
            if let Some(pw) = inner.queue.pop_front() {
                ...
                if let Some(mut sink) = inner.sink.take() {
                    ...
                    inflight = Some(InFlightSend::Write {
                        fut: async move {
                            let result = sink.write(chunk, &mut ctrl).await;
                            (sink, result)
                        }
                        .boxed(), // .boxed() instead of .boxed_local()
                        completion: Some(completion),
                        chunk_size,
                    });
                } else {
        ...
        // Poll inflight operations (same logic, different enum variants)
        if let Some(inflight_op) = &mut inflight {
            match inflight_op {
                InFlightSend::Write {
                    fut, completion, ..
                    ...
                    Poll::Ready((mut sink, result)) => {
                        ...
                        if inner.abort_requested {
                            ...
                            inflight = Some(InFlightSend::Abort {
                                fut: abort_fut,
                                completions,
                            });
                        } else {
                ...
                InFlightSend::Close { fut, completions } => match fut.as_mut().poll(cx) {
                    Poll::Ready(res) => {
                        match &res {
                            Ok(()) => inner.state = StreamState::Closed,
                            Err(err) => {
                ...
                InFlightSend::Abort { fut, completions } => match fut.as_mut().poll(cx) {
                    Poll::Ready(sink_abort_result) => {
                        let notify_result = match sink_abort_result {
                            Ok(()) => Ok(()),
                            Err(sink_error) => {
...
fn update_flags<T, Sink, Strategy: ?Sized + QueuingStrategy<T>>(
    inner: &WritableStreamInner<T, Sink, Strategy>,
...
struct WritableStreamInner<T, Sink, Strategy: ?Sized + QueuingStrategy<T>> {
    state: StreamState,
...
impl<T, Sink, Strategy: ?Sized + QueuingStrategy<T>> WritableStreamInner<T, Sink, Strategy> {
    ...
    fn update_atomic_counters(&self, queue_total_size: &Arc<AtomicUsize>) {
        queue_total_size.store(self.queue_total_size, Ordering::SeqCst);
    }
    ...
    /*fn process_flush_command(
            ...
            let writes_to_wait_for = self.queue.len() + inflight_writes;
    ...
    */
    ...
    fn process_controller_msgs(&mut self, ctrl_rx: &mut UnboundedReceiver<ControllerMsg>) {
        while let Ok(Some(msg)) = ctrl_rx.try_next() {
            match msg {
                ControllerMsg::Error(err) => {
                    self.state = StreamState::Errored;
                    self.abort_reason = Some(format!("Controller error: {:?}", err));
                    self.queue.clear();
...
pub enum _WritableSpawnConfig {
    DefaultThread,
    CustomSpawn(Box<dyn FnOnce(futures::future::BoxFuture<'static, ()>)>),
}

pub enum WritableSpawnConfig {
    Local,
    DefaultThread,
    CustomLocal(Box<dyn FnOnce(LocalBoxFutureStatic<()>) + 'static>),
    CustomSend(Box<dyn FnOnce(futures::future::BoxFuture<'static, ()>) + Send + 'static>),
}

//use futures::future::{BoxFuture, LocalBoxFuture};

pub enum SpawnConfig {
    /// Use local executor (no Send required) - blocks on current thread
    Local,
    /// Use local executor with custom spawn function
    LocalCustom(Box<dyn FnOnce(LocalBoxFuture<'static, ()>) + 'static>),
    /// Use threaded executor (requires Send) - spawns new thread
    Threaded,
    /// Use threaded executor with custom spawn function
    ThreadedCustom(Box<dyn FnOnce(BoxFuture<'static, ()>) + Send + 'static>),
}

impl Default for SpawnConfig {
    fn default() -> Self {
        SpawnConfig::Local // Default to local to avoid forcing Send
    }
}
    ...
    Sink: WritableSink<T> + 'static,
{
    ...
    /// Use local execution (blocks on current thread, no Send required)
    pub fn with_local_execution(mut self) -> Self {
        self.spawn_config = SpawnConfig::Local;
        self
    }

    /// Use local execution with custom spawn function
    pub fn with_local_spawn<F>(mut self, spawn_fn: F) -> Self
    where
        F: FnOnce(LocalBoxFuture<'static, ()>) + 'static,
    {
        self.spawn_config = SpawnConfig::LocalCustom(Box::new(spawn_fn));
        self
    }

    /// Use threaded execution (spawns new thread, requires Send)
    pub fn with_threaded_execution(mut self) -> Self {
        self.spawn_config = SpawnConfig::Threaded;
        self
    }

    /// Use threaded execution with custom spawn function
    pub fn with_threaded_spawn<F>(mut self, spawn_fn: F) -> Self
    where
        F: FnOnce(BoxFuture<'static, ()>) + Send + 'static,
    {
        self.spawn_config = SpawnConfig::ThreadedCustom(Box::new(spawn_fn));
        self
    }

    /// Build the WritableStream with local execution (no Send bounds)
    pub fn build_local(self) -> WritableStream<T, Sink, Unlocked> {
        let strategy: Box<dyn QueuingStrategy<T>> = self
            .strategy
            .unwrap_or_else(|| Box::new(CountQueuingStrategy::new(1)));

        match self.spawn_config {
            SpawnConfig::Local => WritableStream::new_local(self.sink, strategy),
            SpawnConfig::LocalCustom(spawn_fn) => {
                WritableStream::new_with_local_spawn(self.sink, strategy, spawn_fn)
            }
            _ => {
                // Fallback to local for non-local configs when building local
                WritableStream::new_local(self.sink, strategy)
            }
        }
    }

    /// Build the WritableStream with threaded execution (requires Send bounds)
    pub fn build_threaded(self) -> WritableStream<T, Sink, Unlocked>
    where
        T: Send + 'static,
        Sink: Send + 'static,
    {
        // Convert to Send-compatible strategy
        let strategy: Box<dyn QueuingStrategy<T> + Send> = self
            .strategy
            .map(|s| {
                // We can safely cast because we're adding the Send bound at compile time
                // The user calling build_threaded() guarantees T and Sink are Send
                unsafe {
                    std::mem::transmute::<
                        Box<dyn QueuingStrategy<T>>,
                        Box<dyn QueuingStrategy<T> + Send>,
                    >(s)
                }
            })
            .unwrap_or_else(|| Box::new(CountQueuingStrategy::new(1)));
        ...
        match self.spawn_config {
            SpawnConfig::Threaded => WritableStream::new_threaded(self.sink, strategy),
            SpawnConfig::ThreadedCustom(spawn_fn) => {
                WritableStream::new_with_send_spawn(self.sink, strategy, spawn_fn)
            }
            _ => {
                // Fallback to threaded for non-threaded configs when building threaded
                WritableStream::new_threaded(self.sink, strategy)
            }
        }
    }

    /// Build with automatic selection - defaults to local to avoid Send requirements
    /// Use build_threaded() explicitly if you want threaded execution
...
where
    ...
    Sink: WritableSink<T> + 'static,
{
    /// Create a builder for local execution (no Send bounds required)
    pub fn builder_local(sink: Sink) -> WritableStreamBuilder<T, Sink> {
        WritableStreamBuilder::new(sink).with_local_execution()
    }

    /// Create a builder for threaded execution (requires Send bounds)
    pub fn builder_threaded(sink: Sink) -> WritableStreamBuilder<T, Sink>
    where
        T: Send + 'static,
        Sink: Send + 'static,
    {
        WritableStreamBuilder::new(sink).with_threaded_execution()
    }

    /// Generic builder (defaults to local to avoid Send requirements)
    pub fn builder(sink: Sink) -> WritableStreamBuilder<T, Sink> {
        WritableStreamBuilder::new(sink)
    }
}

/*async fn ok_future() -> StreamResult<()> {
    Ok(())
}*/
...
mod tests {
    ...
    impl WritableSink<Vec<u8>> for CountingSink {
        ...
        ) -> impl std::future::Future<Output = StreamResult<()>> + 'static {
    ...
    async fn tokio_spawn_basic_write_test() {
        ...
        let stream = WritableStream::new_with_send_spawn(sink.clone(), strategy, |future| {
            tokio::spawn(future);
        });
        let (_locked_stream, writer) = stream.get_writer().expect("get_writer");
    ...
    async fn backpressure_behavior() {
        ...
        impl WritableSink<Vec<u8>> for SlowSink {
            fn write(
                ...
                _chunk: Vec<u8>,
                _controller: &mut WritableStreamDefaultController,
            ) -> impl Future<Output = StreamResult<()>> + 'static {
...
mod tests_local {
    ...
    // A non-Send sink (contains Rc, which is !Send).
    struct LocalSink {
        state: std::rc::Rc<std::cell::Cell<u32>>,
    }

    impl WritableSink<String> for LocalSink {
        async fn write<'a>(
            &mut self,
            chunk: String,
            _controller: &'a mut WritableStreamDefaultController,
        ) -> StreamResult<()> {
            self.state.set(self.state.get() + chunk.len() as u32);
            Ok(())
        }
    }

    #[test]
    fn test_local_writable_stream() {
        ...
        let mut ws = WritableStream::builder(sink).build_local();
        ...
        // Write some chunks
        {
            ...
            let mut pinned = Pin::new(&mut ws);

            assert!(pinned.as_mut().poll_write(&mut cx, "hi".into()).is_ready());
            assert!(
                pinned
                    .as_mut()
                    .poll_write(&mut cx, "world".into())
                    .is_ready()
            );
        }

        // Verify state got updated
